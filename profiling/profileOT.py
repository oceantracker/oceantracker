         ...
         'base_case_params': {'run_params': {'write_tracks': False},
                             ...
                             'particle_statistics' : [
                                        {'class_name': 'oceantracker.particle_statistics.gridded_statistics.GriddedStats2D_agedBased',
                                         'calculation_interval': 12*3600, 'particle_property_list': ['water_depth'],
                                         'grid_size': [220, 221],
                                        ...
                                        {'class_name': 'oceantracker.particle_statistics.polygon_statistics.PolygonStats2D_ageBased',
                                         'calculation_interval': 12*3600, 'particle_property_list': ['water_depth', 'water_velocity'],
                                         'min_age_to_bin': 0., 'max_age_to_bin': 3. * 24 * 3600, 'age_bin_size': 3600.,
...
if __name__ == '__main__':
    ...
    results_file = 'PIv' + test_version + params['shared_params']['output_file_base'] + '_' + platform.processor().replace(' ', '_').replace(',', '_') + '_' + datetime.now().strftime('%Y-%m-%d')
    ...
    if args.testtype==1:
        ...
        d =  path.join(profile_dir,'pyinstrument')
        makedirs(d,exist_ok=True)
        fnn=  path.join(d, results_file + '_CodeVer_' + ri['code_version_info']['version'])
