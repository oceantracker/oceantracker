...
output_dir = path.join( path.dirname(definitions.ot_root_dir),'oceantracker_output', 'demos')
input_dir = path.join(definitions.ot_root_dir,'tutorials_how_to', 'demo_hindcast')
...
demo_base_params={'output_file_base' : None,
  ...
  'add_date_to_run_output_dir': False,
   ...
   'time_step' : 900,
    'debug': True,
    'reader': {  'input_dir':  path.join(input_dir,'schsim2D'),
...
def make_demo_python(demo_name):
    ...
    run_dir = path.join(path.dirname(__file__))
    ...
    with open(path.join(run_dir,'demo_code', demo_name + '.py'), 'w') as f:
        for l in out:
            f.write(l + "\n")
...
def build_demos(testrun=False):
    ...
    for params in params_list:
        if params is None or type(params)==tuple: continue
        ...
        json_util.write_JSON(path.join(paramdir, demo_name + '.json'), params)
        yaml_util.write_YAML(path.join(paramdir, demo_name + '.yaml'), params)
