...
for nm, m in enumerate(M):
    ...
    nthreads = np.arange(1, nb.get_num_threads(),2)
    ...
    plt.plot(nthreads, time, label=f'work {m}', c = colors[nm], ls = '--')
    plt.plot(nthreads, t1*np.ones(nthreads.size), label=f'work {m}', c = colors[nm], ls = '-')
