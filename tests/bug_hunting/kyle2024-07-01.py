...
if __name__ == '__main__':
    ...
    duration =  5*30*24*3600  # season length 
    ...
    release_interval = 7*24*3600  # seconds between releasing particles 
    output_name = 'kyle_charybdis_full_1yr_500km_V11' # name for the output files
    ...
    # HPC
    if False:
        root_input_dir = f'/hpcfreenas/hindcast/OceanNumNZ-2022-06-20/final_version/' # folder to search for hindcast files, sub-dirs will, by default, also be searched
        root_output_dir = '/hpcfreenas/kyleh'
    else:
        root_input_dir = r'G:\Hindcasts_large\2024_OceanNumNZ-2022-06-20\final_version'  # folder to search for hindcast files, sub-dirs will, by default, also be searched
        root_output_dir = r'F:\OceanTrackerOutput\bug_hunting'
    ...
    ot.settings(output_file_base = output_name,   # name used as base for output files
                ...
                write_tracks = False,
                # max_run_duration = 3600*24*runDurationInDays #365
                )
    ...
    for n, x in enumerate(x0):
             ...
             ot.add_class('release_groups', name = f'P{n:02d}',    # user must provide a name for group first
                 ...
                 release_interval = release_interval,           # seconds between releasing particles - in weeks currently
                 pulse_size = pulse_size,                     # number of particles released each release_interval
                 start = t,
                 max_age = max_age,
                 duration = duration,            # can specify the centre of boundary box for statistics in here for each release group (port)
                 # case = n
                 )                                    # port each has a file 
    ...
    ot.add_class('particle_statistics',
                class_name = 'GriddedStats2D_ageBased',
                 ...
                 grid_size = [200, 200],
                 grid_span = [50000., 50000.],       # meters around release point
                 # age_bin_size = 7*24*3600.,            # bin size in seconds - currently in weeks
                 age_bin_size = max_age,            # bin size in seconds - currently in weeks
