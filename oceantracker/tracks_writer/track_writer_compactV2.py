...
class CompactTracksWriterV2(_BaseWriter):
    def __init__(self):
    ...
    def pre_time_step_write_book_keeping(self):
        ...
        num_alive = self.sel_alive.size
        
        # set up file indices to write to
        self.this_time_step_range = [info['time_particle_steps_written'], info['time_particle_steps_written'] + num_alive]

        # record range if time step in time_particle dim
        nc.file_handle.variables['time_step_range'][nWrite,:] = np.asarray(self.this_time_step_range)
        nc.file_handle.variables['particles_written_per_time_step'][nWrite] =  self.sel_alive.shape[0]

        fi = range(self.this_time_step_range[0], self.this_time_step_range[1])
        nc.file_handle.variables['particle_ID'][fi, ...] = si.class_roles.particle_properties['ID'].get_values(self.sel_alive)

        nc.file_handle.variables['write_step_index'][fi, ...] = info['total_time_steps_written'] * np.ones((num_alive,), dtype=np.int32)
    ...
    def write_time_varying_particle_prop(self, prop_name, data):
        ...
        self.nc.file_handle.variables[prop_name][self.this_time_step_range[0]:self.this_time_step_range[1], ...] = data[self.sel_alive, ...]
