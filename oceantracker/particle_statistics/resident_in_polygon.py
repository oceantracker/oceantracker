...
class ResidentInPolygon(_BaseParticleLocationStats):
    ...
    def initial_setup(self, **kwargs):
        ...
        self.sum_residence_time = np.full(s, 0., np.float64)
        self.count_residence_time  = np.full(s, 0, np.int64)
    ...
    def do_counts(self,n_time_step, time_sec, sel, alive):
        ...
        self._sum_residence_times(params['update_interval'], # interval is time step
                was_inside.data,
                inside.data,
                part_prop['IDrelease_group'].data,
                part_prop[info['entry_count_prop']].data,
                self.sum_residence_time[nWrite,:], self.count_residence_time[nWrite, :],
    ...
    @staticmethod
    @njitOT
    def _sum_residence_times(time_step, was_inside,inside,
                                IDrelease_group,
                                entry_count,
                                sum_residence_time,count_residence_time,
        ...
        for n in sel:
            ...
            if inside[n] >= 0:
                if was_inside[n] < 0 : entry_count[n] += 1

                if entry_count[n] == 1 :
                    # only count first entry
                    sum_residence_time[ng] += time_step
                    count_residence_time[ng] += 1
                # count all time inside
                sum_exposure_time[ng] += time_step
                count_exposure_time[ng] += 1

            elif was_inside[n] >= 0:
                # was inside but now outside, add last increment
                if entry_count[n] == 1 :
                    sum_residence_time[ng] += time_step
                    count_residence_time[ng] += 1
                sum_exposure_time[ng] += time_step
