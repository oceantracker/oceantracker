...
class _BaseEventLogger(ParameterBaseClass):
    ...
    def __init__(self):
        ...
        self.add_default_params({'output_file_base': PVC('event_logger',str),
                                 'write': PVC(True,bool),
    ...
    def set_up_output_file(self,addition_prop_to_write = None):
        ...
        info['output_file'] = self.params['output_file_base'] + '_' + self.params['name'] + '.nc'
        ...
        for n, d in enumerate(vec_dims): self.nc.create_dimension(d, dim_size=n + 1)

        self.nc.create_variable('event_flag', ['event_dim'], np.int8, description='event started=1, ended = -1', chunksizes=[chunk])
        self.nc.create_variable('time', ['event_dim'], np.float64, description='time in seconds since 1970-01-01', chunksizes=[chunk])
