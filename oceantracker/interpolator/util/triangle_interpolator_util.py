...
@njit
def _kernal_get_depth_cell_time_varying_Slayer(xq, grid, part_prop, step_info, n):
    ...
    if part_prop['status'][n] == status_stranded_by_tide:
        ...
        z_below = _eval_z_at_nz_cell(step_info['fractional_time_steps'], step_info['nb'], bottom_nz_cell, grid['zlevel'], grid['bottom_cell_index'], part_prop['bc_cords'][n, :], nodes)
        xq[2] = z_below
    ...
    z_below = _eval_z_at_nz_cell(step_info['fractional_time_steps'], nb, nz, grid['zlevel'],grid['bottom_cell_index'],part_prop['bc_cords'][n, :], nodes)
    ...
    if zq >= z_below:
        ...
        z_above = _eval_z_at_nz_cell(step_info['fractional_time_steps'], nb, nz + 1,  grid['zlevel'], grid['bottom_cell_index'], part_prop['bc_cords'][n, :], nodes)
        while zq > z_above:
            ...
            z_above = _eval_z_at_nz_cell(step_info['fractional_time_steps'], nb, nz, grid['zlevel'], grid['bottom_cell_index'], part_prop['bc_cords'][n, :], nodes)
            n_vertical_steps += 1
    else:
        # search downwards
        z_above  = z_below
        z_below = _eval_z_at_nz_cell(step_info['fractional_time_steps'], nb, nz - 1, grid['zlevel'], bottom_nz_nodes, part_prop['bc_cords'][n, :], nodes)
        while zq < z_below:
...
def combined_cell_and_depth_walk(xq, grid, part_prop, active, step_info):
