...
class Solver(ParameterBaseClass):
    ...
    def solveV1(self):
        ...
        for n_time_step  in range(nt1, model_times.size-1):
            ...
            new_particle_indices = pgm.release_particles(n_time_step, t1)

            # do stats etc updates and write tracks at new time step
            self._pre_step_bookkeeping(n_time_step, t1, new_particle_indices)

            if si.settings.restart_interval is not None and self.schedulers['save_state'].do_task(n_time_step):
                self._save_state_for_restart(n_time_step, t1)
            ...
            if n_time_step % nt_write_time_step_to_screen == 0:
                self._screen_output(n_time_step, t1, t0_model, perf_counter() - t0_step)
            ...
            # random walk dispersion is done by adding to velocity modifier prior to integration step
    ...
    def solve(self):
        ...
        for n_time_step  in range(nt1, model_times.size-1):
            ...
            if abs(t2 - ri.start_time) > ri.duration: break
            if si.settings.throw_debug_error == 1 and nt1 >= int(0.2*model_times.size):
                raise(Exception(f'Restart testing, throwing deliberate error at {time_util.seconds_to_isostr(t1)}'))
    ...
    def _pre_step_bookkeeping(self, n_time_step,time_sec,
        ...
        print('xx1','x', part_prop['x'].data[:4, 2], 'tide',  part_prop['tide'].data[:4], 'diff', np.abs(part_prop['x'].data[:4, 2] - part_prop['tide'].data[:4]) )
        print('\t xx1a','depth', part_prop['water_depth'].data[:4],'bc', part_prop['bc_coords'].data[0],'xy0',part_prop['x'].data[0, :2])
        print('\t xx1b  ', fgm.info['fractional_time_steps'], 'nz', part_prop['nz_cell'].data[:4],  'zfrac', part_prop['z_fraction'].data[:4])
        ...
        print('xx2','x', part_prop['x'].data[:4, 2], 'tide',  part_prop['tide'].data[:4], 'diff', np.abs(part_prop['x'].data[:4, 2] - part_prop['tide'].data[:4])  )
        print('\t xx2a', 'depth', part_prop['water_depth'].data[:4],'bc', part_prop['bc_coords'].data[0],'xy0',part_prop['x'].data[0, :2])
        print('\t xx2b ', fgm.info['fractional_time_steps'], 'nz', part_prop['nz_cell'].data[:4],  'zfrac', part_prop['z_fraction'].data[:4])
    ...
    def _pre_step_bookkeepingV1(self, n_time_step,time_sec,
                              new_particle_indices=np.full((0,),0,dtype=np.int32)):
        part_prop = si.class_roles.particle_properties
        pgm = si.core_class_roles.particle_group_manager
        fgm = si.core_class_roles.field_group_manager
        ...
        junk = part_prop['tide'].data.copy()
        fgm.interp_field_at_particle_locations('tide', alive, output=junk)
        ...
        print('xx1','x', part_prop['x'].data[:4, 2], 'tide',  part_prop['tide'].data[:4], 'diff', np.abs(part_prop['x'].data[:4, 2] - part_prop['tide'].data[:4]) )
        print('xx1a','depth', part_prop['water_depth'].data[:4],'bc', part_prop['bc_coords'].data[:4])
        print('xx1b  ', fgm.info['fractional_time_steps'], 'nz', part_prop['nz_cell'].data[:4],  'zfrac', part_prop['z_fraction'].data[:4])
        fgm.update_dry_cell_values()
        ...
        print('xx2','x', part_prop['x'].data[:4, 2], 'tide',  part_prop['tide'].data[:4], 'diff', np.abs(part_prop['x'].data[:4, 2] - part_prop['tide'].data[:4])  )
        print('xx2a', 'depth', part_prop['water_depth'].data[:4],'bc', part_prop['bc_coords'].data[:4])
        print('xx2b ', fgm.info['fractional_time_steps'], 'nz', part_prop['nz_cell'].data[:4],  'zfrac', part_prop['z_fraction'].data[:4])
